...
export type UiStrings = {
  ...
  readonly hostCatalogUnavailable: string
  ...
  readonly ailySource: string
  readonly arduinoSource: string
  ...
  failed(action: 'load' | 'install', source: LibrarySource, detail: string): string
}

const EN: UiStrings = {
  panelTitle: 'Library Manager', ailyTab: 'Aily Libraries', arduinoTab: 'Arduino Official', preferred: 'Preferred',
  ...
  hostCatalogUnavailable: 'Waiting for the main application Library Manager catalog…',
  ...
  otherArchitecture: 'Other architecture', ailySource: 'Main application Library Manager',
  arduinoSource: 'Arduino Library Manager', resultUnit: 'libraries',
  ...
  failed: (action, source, detail) => `${action === 'load'
    ...
    : 'Failed to install the library'}: ${detail}`,
}

const ZH_CN: UiStrings = {
  panelTitle: '库管理', ailyTab: 'Aily 库', arduinoTab: 'Arduino 官方库', preferred: '首选',
  searchAily: '搜索 Aily 库', searchArduino: '搜索 Arduino 官方库', refresh: '刷新',
  add: '安装', adding: '正在安装…', installed: '已安装',
  loadingAily: '正在加载 Aily 库…', loadingArduino: '正在加载 Arduino 官方库…',
  emptyAily: '没有匹配的 Aily 库', emptyArduino: '没有匹配的 Arduino 官方库',
  unavailable: '当前 Coder 工程尚未就绪。', hostCatalogUnavailable: '正在等待主软件“库管理”目录…',
  ...
  compatible: '兼容当前平台', otherArchitecture: '其他架构', ailySource: '主软件库管理',
  arduinoSource: 'Arduino Library Manager', resultUnit: '个库', endOfResults: '已显示全部匹配库',
...
const ZH_HK: UiStrings = {
  ...
  compatible: '相容目前平台', otherArchitecture: '其他架構', ailySource: '主軟體函式庫管理',
  arduinoSource: 'Arduino Library Manager', resultUnit: '個函式庫', endOfResults: '已顯示所有相符函式庫',
...
function localized(
  overrides: Partial<Omit<UiStrings, 'added' | 'failed'>>,
  messages: Pick<UiStrings, 'added' | 'failed'>,
): UiStrings {
  return { ...EN, ...overrides, ...messages }
}

const JA = localized({
  panelTitle: 'ライブラリ管理', ailyTab: 'Aily ライブラリ', arduinoTab: 'Arduino 公式', preferred: '推奨',
  searchAily: 'Aily ライブラリを検索', searchArduino: 'Arduino 公式ライブラリを検索', refresh: '更新',
  add: 'インストール', adding: 'インストール中…', installed: 'プロジェクト内',
  ...
  unavailable: 'Coder プロジェクトの準備ができていません。', hostCatalogUnavailable: 'メインアプリのライブラリカタログを待機中…',
  ...
  compatible: '互換', otherArchitecture: '別のアーキテクチャ', ailySource: 'メインアプリのライブラリマネージャー',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `${source === 'aily' ? 'Aily' : 'Arduino'} ライブラリの読み込みに失敗しました` : 'ライブラリのインストールに失敗しました'}：${detail}`,
})

const KO = localized({
  panelTitle: '라이브러리 관리', ailyTab: 'Aily 라이브러리', arduinoTab: 'Arduino 공식', preferred: '권장',
  searchAily: 'Aily 라이브러리 검색', searchArduino: 'Arduino 공식 라이브러리 검색', refresh: '새로고침',
  add: '설치', adding: '설치 중…', installed: '프로젝트에 있음',
  ...
  unavailable: 'Coder 프로젝트가 준비되지 않았습니다.', hostCatalogUnavailable: '메인 앱 라이브러리 카탈로그를 기다리는 중…',
  ...
  compatible: '호환됨', otherArchitecture: '다른 아키텍처', ailySource: '메인 앱 라이브러리 관리자',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `${source === 'aily' ? 'Aily' : 'Arduino'} 라이브러리를 불러오지 못했습니다` : '라이브러리를 설치하지 못했습니다'}: ${detail}`,
})

const DE = localized({
  panelTitle: 'Bibliotheksverwaltung', ailyTab: 'Aily-Bibliotheken', arduinoTab: 'Arduino offiziell', preferred: 'Bevorzugt',
  searchAily: 'Aily-Bibliotheken durchsuchen', searchArduino: 'Offizielle Arduino-Bibliotheken durchsuchen', refresh: 'Aktualisieren',
  add: 'Installieren', adding: 'Wird installiert…', installed: 'Im Projekt',
  ...
  unavailable: 'Das Coder-Projekt ist noch nicht bereit.', hostCatalogUnavailable: 'Warten auf den Bibliothekskatalog der Hauptanwendung…', toggle: 'Bibliotheksliste ein- oder ausblenden',
  ...
  compatible: 'Kompatibel', otherArchitecture: 'Andere Architektur', ailySource: 'Bibliotheksverwaltung der Hauptanwendung',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `${source === 'aily' ? 'Aily' : 'Arduino'}-Bibliotheken konnten nicht geladen werden` : 'Die Bibliothek konnte nicht installiert werden'}: ${detail}`,
})

const FR = localized({
  panelTitle: 'Gestionnaire de bibliothèques', ailyTab: 'Bibliothèques Aily', arduinoTab: 'Arduino officiel', preferred: 'Préféré',
  ...
  add: 'Installer', adding: 'Installation…', installed: 'Dans le projet',
  ...
  unavailable: 'Le projet Coder n’est pas prêt.', hostCatalogUnavailable: 'En attente du catalogue de bibliothèques de l’application principale…', toggle: 'Développer ou réduire la liste des bibliothèques',
  ...
  compatible: 'Compatible', otherArchitecture: 'Autre architecture', ailySource: 'Gestionnaire de bibliothèques principal',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `Échec du chargement des bibliothèques ${source === 'aily' ? 'Aily' : 'Arduino'}` : 'Échec de l’installation de la bibliothèque'} : ${detail}`,
})

const ES = localized({
  panelTitle: 'Gestor de bibliotecas', ailyTab: 'Bibliotecas Aily', arduinoTab: 'Arduino oficial', preferred: 'Preferida',
  searchAily: 'Buscar bibliotecas Aily', searchArduino: 'Buscar bibliotecas oficiales de Arduino', refresh: 'Actualizar',
  add: 'Instalar', adding: 'Instalando…', installed: 'En el proyecto',
  ...
  unavailable: 'El proyecto Coder aún no está listo.', hostCatalogUnavailable: 'Esperando el catálogo de bibliotecas de la aplicación principal…', toggle: 'Expandir o contraer la lista de bibliotecas',
  ...
  compatible: 'Compatible', otherArchitecture: 'Otra arquitectura', ailySource: 'Administrador de bibliotecas principal',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `No se pudieron cargar las bibliotecas ${source === 'aily' ? 'Aily' : 'Arduino'}` : 'No se pudo instalar la biblioteca'}: ${detail}`,
})

const PT = localized({
  panelTitle: 'Gerenciador de bibliotecas', ailyTab: 'Bibliotecas Aily', arduinoTab: 'Arduino oficial', preferred: 'Preferida',
  searchAily: 'Pesquisar bibliotecas Aily', searchArduino: 'Pesquisar bibliotecas oficiais do Arduino', refresh: 'Atualizar',
  add: 'Instalar', adding: 'Instalando…', installed: 'No projeto',
  ...
  unavailable: 'O projeto Coder ainda não está pronto.', hostCatalogUnavailable: 'Aguardando o catálogo de bibliotecas do aplicativo principal…', toggle: 'Expandir ou recolher a lista de bibliotecas',
  ...
  compatible: 'Compatível', otherArchitecture: 'Outra arquitetura', ailySource: 'Gerenciador de bibliotecas principal',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `Falha ao carregar bibliotecas ${source === 'aily' ? 'Aily' : 'Arduino'}` : 'Falha ao instalar a biblioteca'}: ${detail}`,
})

const RU = localized({
  panelTitle: 'Менеджер библиотек', ailyTab: 'Библиотеки Aily', arduinoTab: 'Официальные Arduino', preferred: 'Рекомендуется',
  searchAily: 'Поиск библиотек Aily', searchArduino: 'Поиск официальных библиотек Arduino', refresh: 'Обновить',
  add: 'Установить', adding: 'Установка…', installed: 'В проекте',
  ...
  unavailable: 'Проект Coder ещё не готов.', hostCatalogUnavailable: 'Ожидание каталога библиотек основного приложения…', toggle: 'Развернуть или свернуть список библиотек',
  ...
  compatible: 'Совместима', otherArchitecture: 'Другая архитектура', ailySource: 'Менеджер библиотек основного приложения',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `Не удалось загрузить библиотеки ${source === 'aily' ? 'Aily' : 'Arduino'}` : 'Не удалось установить библиотеку'}: ${detail}`,
})

const AR = localized({
  panelTitle: 'مدير المكتبات', ailyTab: 'مكتبات Aily', arduinoTab: 'Arduino الرسمية', preferred: 'مفضلة',
  searchAily: 'البحث في مكتبات Aily', searchArduino: 'البحث في مكتبات Arduino الرسمية', refresh: 'تحديث',
  add: 'تثبيت', adding: 'جارٍ التثبيت…', installed: 'في المشروع',
  ...
  unavailable: 'مشروع Coder غير جاهز بعد.', hostCatalogUnavailable: 'في انتظار فهرس مكتبات التطبيق الرئيسي…', toggle: 'توسيع قائمة المكتبات أو طيها',
  ...
  compatible: 'متوافقة', otherArchitecture: 'بنية أخرى', ailySource: 'مدير مكتبات التطبيق الرئيسي',
  ...
  failed: (action, source, detail) => `${action === 'load' ? `تعذر تحميل مكتبات ${source === 'aily' ? 'Aily' : 'Arduino'}` : 'تعذر تثبيت المكتبة'}: ${detail}`,
})
