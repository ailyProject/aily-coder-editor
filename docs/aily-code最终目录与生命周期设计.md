...
> **当前实现覆盖（2026-09-09）**：项目根 `package.json` 保存全部 Coder 工程配置，可编辑源码和本地库位于持久化 `sketch/` 工作区。Coder 新建复制共享 `board-*` 主板包的 `template_arduino/package.json`，并将保持原始内容的源码模板 `template_arduino/project.aci` 复制为 `sketch/src/main.cpp`；Aily 与 Arduino 官方 npm 库保留在包内并映射最终 `src` 库根，只有需要修改时才复制到 `sketch/libraries/`。下文根 `project.aci` 等旧目录示意仅保留为设计历史。
